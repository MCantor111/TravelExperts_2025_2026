/* 
  ...
  Description:
    ...
    - Handles registration, booking, and package/agency display.
    - Provides inline validation, formatted inputs, and dynamic popups.
  ============================================
*/
...
function validateRegistration() {
  ...
  for(const id of required){
    const f=document.getElementById(id);
    if(!f.value.trim()) showError(id,"This field is required.");
  }
  ...
  if(busPhone && !phoneRe.test(busPhone)) showError("custBusPhone","Use format: 403-555-8192");
...
// =========================
// REGISTRATION POPUP (PERSISTENT MODAL)
// =========================
function showRegisterPopup(firstName) {
  const popup = document.getElementById("registerPopup");
  const nameSpan = document.getElementById("popupName");
  if (!popup || !nameSpan) {
    console.warn("Popup element missing in DOM");
    return;
  }

  nameSpan.textContent = " " + firstName + "!"; // adds extra space after comma
...
function setupOrderForm() {
  ...
  orderForm.addEventListener("submit",async e=>{
    ...
    try{
      ...
      const result=await res.json();
      if(!result.ok){ showPopup(`⚠️ ${result.message||"Booking failed."}`,false); return; }
